...
public class DemoAutoBoxing {
	/**
	 * Inner MyItem class represents a consumer item for purchase
	 * @author dpeters
	 *
	 */
	private class MyItem implements Comparable<MyItem>, Comparator<MyItem>{
		private int id;
		 ...
		 */
		@Override
		public int compareTo(MyItem someOtherItem) {
			/**
...
//			return this.getName().compareTo(o.getName());
		 ...
		 * @return		integer metric
		 */
		public int compareById(MyItem o1, MyItem o2) {
...
//		List<String> names = Arrays.asList(a);	// make array a FIXED SIZE LIST
	...
	public void addItems() {
		...
		List<MyItem> items = new ArrayList<DemoAutoBoxing.MyItem>(Arrays.asList(
				new MyItem(1,3.49,"OJ"),
				new MyItem(3,1.49,"Bread"),
				new MyItem(2,2.49,"Milk")
				));
		...
		items.sort(this.new MyItem()); // Item class implements Comparator interface (compare method)
		System.out.println(items.size() + " items in list.");
		for (MyItem item : items) {
			System.out.println(item); 	// call toString()
		}
		System.out.println("Sort Item object BY NAME REVERSED (Specific Item Comparator Order)...");
		items.sort(this.new MyItem().reversed()); // Item class implements Comparator interface (compare method)
		System.out.println(items.size() + " items in list.");
		for (MyItem item : items) {
			System.out.println(item); 	// call toString()
		}
		 ...
		 */
		items.sort(Comparator.comparing(MyItem::getId)); // Key Extractor is a Getter (Lambda Method Reference)
		System.out.println(items.size() + " items in list.");
		for (MyItem item : items) {
			System.out.println(item); 	// call toString()
		}
		System.out.println("Sort Item object BY PRICE (Specific Comparator Order)...");
		items.sort(Comparator.comparing(MyItem::getPrice)); // Key Extractor is a Getter (Lambda Method Reference)
		System.out.println(items.size() + " items in list.");
		for (MyItem item : items) {
			System.out.println(item); 	// call toString()
		}
		System.out.println("Sort Item object BY NAME REVERSE (Specific Comparator Order)...");
		items.sort(Comparator.comparing(MyItem::getName).reversed()); // Key Extractor is a Getter (Lambda Method Reference)
		System.out.println(items.size() + " items in list.");
		for (MyItem item : items) {
			System.out.println(item); 	// call toString()
		}
		System.out.println("Sort Item object BY NAME (Specific Anonymous Inner class Comparator Order)...");
		items.sort(new Comparator<MyItem>() {
